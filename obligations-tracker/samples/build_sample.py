...
def main() -> None:
    ...
    for deal, outcomes, role in DEALS:
        t = track(deal, source="samples/portfolio.py", is_sample=True)
        t.outcomes = outcomes
        t.fund_role = role
        portfolio.append(t.model_dump(mode="json"))
    (HERE / "portfolio-tracked.json").write_text(json.dumps(portfolio, indent=2) + "\n")
    ...
    demo.write_text("// Generated by samples/build_sample.py. Do not edit.\n"
                    ...
                    f"window.DEALS = {json.dumps(portfolio, indent=2)};\n")
    n = sum(len(d["obligations"]) for d in portfolio)
    print(f"{len(portfolio)} deals, {n} obligations -> samples/*.json, demo/data.js")
